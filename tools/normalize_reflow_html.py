...
def normalize_html(path: Path) -> bool:
    ...
    if path.name == "目次_reflow.html":
        ...
        s = re.sub(
            r'<nav class="(?:epub|reflow)-toc" id="toc">',
            '<nav class="reflow-toc" id="toc" epub:type="toc">',
            s,
            count=1,
        )
    else:
...
def main() -> None:
    changed = []
    targets = [TARGET_DIR / "目次_reflow.html"] + sorted(TARGET_DIR.glob("*/*_reflow.html"))
    for path in targets:
