...
- 09〜17：`REWRITTEN_PENDING_USER`。9本のMDを形式別に改稿。10・16は画像各1点を選定・保存・本文挿入済み。
...
| 09 | `09_春の女騎士配属名鑑` | REWRITTEN_PENDING_USER | 五人の配属カード・持ち物・失敗談・短問答へ改稿。新しい配属細部は未承認ドラフト。旧HTML未同期 |
| 10 | `10_花見ダンジョン危険案内` | REWRITTEN_PENDING_USER | 地下桜の六区間・70分・料金・引返し条件へ改稿。画像1点挿入済み。旧HTML未同期 |
| 11 | `11_新人パーティー選手名鑑` | REWRITTEN_PENDING_USER | 八組の実績・持ち物・依頼条件をカード化。流星猫の代表・構成は確認待ちのため列記保留。旧HTML未同期 |
| 12 | `12_新生活アーティファクトカタログ` | REWRITTEN_PENDING_USER | 十商品の買値・維持費・実演条件と買物例を整備。旧HTML未同期 |
| 13 | `13_王都古道具屋春の掘り出し市` | REWRITTEN_PENDING_USER | 猫背堂の一点物8点を来歴・欠点・価格の広告に再構成。旧HTML未同期 |
| 14 | `14_春のあやしさ満天堂` | REWRITTEN_PENDING_USER | 商品8点、値引きセット、送料、注文欄の通販広告へ改稿。旧HTML未同期 |
| 15 | `15_今日の魔術春眠編` | REWRITTEN_PENDING_USER | ネムラの四つの実演と三通の質問。作用と期待外れを分ける連載へ改稿。旧HTML未同期 |
| 16 | `16_魔物の育て方新入生編` | REWRITTEN_PENDING_USER | ポポの三週間の成長・六種比較・初期費用と月額へ改稿。画像1点挿入済み。旧HTML未同期 |
| 17 | `17_ロブロンス春号` | REWRITTEN_PENDING_USER | 貼り紙・小商い・18枚の集計など雑多10項目へ改稿。前号の闘士記事へ戻していない。旧HTML未同期 |
...
| 記事 | 画像状態 | 保存・配置 |
|---|---|---|
| 10 花見ダンジョン | IMAGE_SELECTED | `hanami_quarry_01.png`。導入後のfigure。試作1組、単体2候補を評価し1点採用 |
| 16 魔物の育て方 | IMAGE_SELECTED | `moco_outgrown_basket_01.png`。飼育帳後のfigure。試作2組、単体1候補を評価し1点採用 |

09・11〜15・17は、今次は人物・商品の比較カード、数字、貼り紙などの本文構成を優先し、画像を生成していない。人物外見などを無根拠に固定する汎用絵は追加しない。本文確認後、画像の追加が有効か再判断できる。
