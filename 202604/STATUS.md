# 202604号 現況表

## 2026-09-15 追加画像6候補の整理・花見記事5点の配置

202604/images に追加された6候補を確認し、「聖職者花見乱痴気騒ぎ」用の5点を内容に合う名前へ変更して、正本Markdownの対応段落へ配置しました。勘定問答は本文どおり左手の串が5本の版を採用し、串6本の旧候補は説明的な名前でimagesに保管しました。ファイルの画素・圧縮・解像度は変更していません。

不足一覧から完了5項目を削除し、初期計画150点中22点完了、残り128点（本文125、表紙1、条件付き2）です。計画外の桜花全身画1点と初期の既存5点を合わせ、本文参照画像は28点です。記事別プロンプトの状態・採用ファイルと[画像配置記録](画像配置記録.md)も更新しました。

次の本文記事は05_引っ越し魔導便トラブル告発です。00表紙も不足しています。今回の更新は正本Markdownへの配置までで、固定HTML・EPUB・最終誌面の面積比確認は未実施です。

以下は前回までの作業記録です。

## 2026-09-15 生成済み画像の配置

202604/images の19点を確認・リネームしました。計画内17点（01制服名鑑5点、02ギャルギルド7点、03登録所5点）と計画外の桜花全身画1点を、対応する正本Markdownへ配置しました。木片が9枚の旧候補1点は未掲載で保管し、登録所には本文どおり12枚の修正版を採用しています。

配置済み17項目を[画像不足一覧](画像不足一覧.md)から削除しました。残りは133点（本文130点、表紙1点、条件付き2点）です。対応する記事別プロンプトも採用ファイルと配置状態を更新しました。旧名・新名・挿入箇所は[画像配置記録](画像配置記録.md)を参照してください。

記事の既存文章・見出し・表・既存画像は保持しています。今回の更新は正本Markdownへの画像配置までです。固定HTML・EPUBは未同期で、最終誌面の画像面積比は組版時の確認が必要です。以下の過去の日付の記録は当時の状態を示します。

このファイルは `202604/HANDOFF.md` とセットで使う。

ここでいう状態は **構造・編集作業上の状態** であり、掲載確定・内容承認を意味しない。

## 2026-09-14 画像不足一覧・記事別プロンプト

今回の指示に基づき、画像面積が常に文字面積を上回る前提へ画像計画を更新しました。[画像不足一覧](画像不足一覧.md)に全体判断・全150カット・配置案をまとめ、全29フォルダのプロンプト.mdに候補または本文未存在の記録を配置しています。

現行本文24記事は追加147点がIMAGE_PLANNED、表紙は1点がIMAGE_PLANNEDです。24_春祭り警備隊レポートの2点は短いHTMLに基づくPLANNED_CONDITIONALです。起動用だけの19実行委員会・22南区花見宿事件・24襲撃はBLOCKED_NO_BODYとし、現行記事のプロンプトへの参照を置きました。

既存画像は01の3点、10・16の各1点を維持しています。10・16の既存IMAGE_SELECTEDと追加IMAGE_PLANNEDは併存します。本文の承認状態はそのままです。今回の画像生成・本文挿入・レイアウト同期・全号ビルドは未実施です。以下の2026-09-06節と従来の画像状態表は当時の作業記録です。

## 2026-09-06 第2回編集後

全29フォルダを監査済み。監査時の本文正本は24記事、現行企画は8本。起動用ファイルだけのフォルダが3件、HTMLのみが1件ある。引き渡し時の表で「現行原稿あり」と読めた箇所は実査結果で補正した。

- 全号監査・改稿優先順位・番号衝突の比較：[`EDITORIAL_AUDIT.md`](EDITORIAL_AUDIT.md)
- 改稿内容・整合確認・次回開始点：[`EDIT_LOG.md`](EDIT_LOG.md)
- 03〜08：`REWRITTEN_PENDING_USER`。MDを記事別に改稿。ユーザー未確認、派生HTML未同期。
- 09〜17：`REWRITTEN_PENDING_USER`。9本のMDを形式別に改稿。10・16は画像各1点を選定・保存・本文挿入済み。
- それ以外：本文・既存HTMLの監査のみ。全号は未校了。
- 01の薔薇十字設定、11と23の人物、21の時系列、23の依頼難度、番号衝突に未解決事項あり。

## 状態記号

- `MISSING_BODY`：現行本文Markdownなし。掲載確定には正本が必要
- `COVER`：表紙
- `DRAFT_AI_UNREVIEWED`：AIドラフト。ユーザー編集済みとみなさない
- `CURRENT_CONCEPT`：現行企画として意図的に差し替えられている。旧版へ戻さない
- `REVIEW_REQUIRED`：現行原稿はあるが、内容・密度・誌面性を再評価する
- `NUMBER_CONFLICT`：同じ番号または近接する重複候補がある。勝手に削除・改番しない

## 現行ディレクトリ一覧

| No. | 現行ディレクトリ | 状態 | 現在の編集上の注意 |
|---:|---|---|---|
| 00 | `00_表紙` | COVER / AUDITED | 10にない失踪事件の煽りあり。本文確定後に調整 |
| 01 | `01_王都女学院春の制服名鑑` | AUDITED / REVIEW_REQUIRED | 三校比較を維持。薔薇十字の学校設定と現行本文に差。軽量版表記・固定版別文 |
| 02 | `02_はじめてのギャルギルド` | CURRENT_CONCEPT / AUDITED | 現行企画を維持。冒頭・アムロミレイ・プリウラを保護。HTML差分あり |
| 03 | `03_王都ギルド登録所大混乱` | REWRITTEN_PENDING_USER | MD改稿・自己校閲済み。ユーザー未確認、旧HTML未同期。EDIT_LOG参照 |
| 04 | `04_聖職者花見乱痴気騒ぎ` | REWRITTEN_PENDING_USER / IMAGE_SELECTED | 画像5点を正本MDへ配置済み。本文はユーザー未確認、旧HTML未同期。記事別プロンプト・画像配置記録参照 |
| 05 | `05_引っ越し魔導便トラブル告発` | REWRITTEN_PENDING_USER | MD改稿・自己校閲済み。ユーザー未確認、旧HTML未同期。EDIT_LOG参照 |
| 06 | `06_春の闇バイト求人特集` | REWRITTEN_PENDING_USER | MD改稿・自己校閲済み。ユーザー未確認、旧HTML未同期。EDIT_LOG参照 |
| 07 | `07_王都下宿ぼったくり事情` | REWRITTEN_PENDING_USER | MD改稿・自己校閲済み。ユーザー未確認、旧HTML未同期。EDIT_LOG参照 |
| 08 | `08_脱法スキル講習会の実態` | REWRITTEN_PENDING_USER | MD改稿・自己校閲済み。ユーザー未確認、旧HTML未同期。EDIT_LOG参照 |
| 09 | `09_春の女騎士配属名鑑` | REWRITTEN_PENDING_USER | 五人の配属カード・持ち物・失敗談・短問答へ改稿。新しい配属細部は未承認ドラフト。旧HTML未同期 |
| 10 | `10_花見ダンジョン危険案内` | REWRITTEN_PENDING_USER | 地下桜の六区間・70分・料金・引返し条件へ改稿。画像1点挿入済み。旧HTML未同期 |
| 11 | `11_新人パーティー選手名鑑` | REWRITTEN_PENDING_USER | 八組の実績・持ち物・依頼条件をカード化。流星猫の代表・構成は確認待ちのため列記保留。旧HTML未同期 |
| 12 | `12_新生活アーティファクトカタログ` | REWRITTEN_PENDING_USER | 十商品の買値・維持費・実演条件と買物例を整備。旧HTML未同期 |
| 13 | `13_王都古道具屋春の掘り出し市` | REWRITTEN_PENDING_USER | 猫背堂の一点物8点を来歴・欠点・価格の広告に再構成。旧HTML未同期 |
| 14 | `14_春のあやしさ満天堂` | REWRITTEN_PENDING_USER | 商品8点、値引きセット、送料、注文欄の通販広告へ改稿。旧HTML未同期 |
| 15 | `15_今日の魔術春眠編` | REWRITTEN_PENDING_USER | ネムラの四つの実演と三通の質問。作用と期待外れを分ける連載へ改稿。旧HTML未同期 |
| 16 | `16_魔物の育て方新入生編` | REWRITTEN_PENDING_USER | ポポの三週間の成長・六種比較・初期費用と月額へ改稿。画像1点挿入済み。旧HTML未同期 |
| 17 | `17_ロブロンス春号` | REWRITTEN_PENDING_USER | 貼り紙・小商い・18枚の集計など雑多10項目へ改稿。前号の闘士記事へ戻していない。旧HTML未同期 |
| 18 | `18_王都新歓コンパ危険地帯レポート` | AUDITED / DRAFT_AI_UNREVIEWED | 現場レポートとして場所・時間・団体ごとの差を作る |
| 19 | `19_春祭りガイド` | NUMBER_CONFLICT / AUDITED | 同番号の記事あり。内容比較前に削除・改番しない |
| 19 | `19_春祭り実行委員会の闇` | NUMBER_CONFLICT / AUDITED / MISSING_BODY | 本文・企画・HTMLなし。preview起動用2ファイルのみ |
| 20 | `20_王都東門荷馬車横転事件` | AUDITED / DRAFT_AI_UNREVIEWED | 後半事件簿。事件固有の因果・目撃者・積み荷を強化候補 |
| 21 | `21_魔術学院入学式失踪事件` | AUDITED / DRAFT_AI_UNREVIEWED | 後半事件簿。単なる噂一覧で終わらせず事件の芯を確認 |
| 22 | `22_王都南区花見宿事件` | NUMBER_CONFLICT / AUDITED / MISSING_BODY | 本文・企画・HTMLなし。preview起動用2ファイルのみ |
| 22 | `22_花見宿ガイド` | NUMBER_CONFLICT / AUDITED | 同番号の記事あり。ガイド記事としての役割を比較 |
| 23 | `23_新人冒険者パーティー流星猫事件` | AUDITED / DRAFT_AI_UNREVIEWED | パーティー「流星猫」を固有キャラ・騒動として成立させる |
| 24 | `24_春祭り警備隊レポート` | NUMBER_CONFLICT / AUDITED / MISSING_BODY | 警備実務の短いHTMLのみ。独立掲載なら正本MDから制作 |
| 24 | `24_春祭り警備隊襲撃事件` | NUMBER_CONFLICT / AUDITED / MISSING_BODY | 本文・企画・HTMLなし。25と二稿がある状態ではない |
| 25 | `25_春祭り警備隊襲撃事件` | NUMBER_CONFLICT / AUDITED | 本文と企画あり。HTMLなし。負傷者・不明者と証言の整理が必要 |

---

## 画像の状態（本文の承認状態とは別）

| 記事 | 画像状態 | 保存・配置 |
|---|---|---|
| 10 花見ダンジョン | IMAGE_SELECTED | `hanami_quarry_01.png`。導入後のfigure。試作1組、単体2候補を評価し1点採用 |
| 16 魔物の育て方 | IMAGE_SELECTED | `moco_outgrown_basket_01.png`。飼育帳後のfigure。試作2組、単体1候補を評価し1点採用 |

09・11〜15・17は、今次は人物・商品の比較カード、数字、貼り紙などの本文構成を優先し、画像を生成していない。人物外見などを無根拠に固定する汎用絵は追加しない。本文確認後、画像の追加が有効か再判断できる。

## 既知の構造上の注意

### 1. 旧企画書の番号は現況と一致しない

`docs/07_202604_issue_plan.md` と `docs/08_202604_volume_expansion_spec.md` は企画背景として有用だが、現在のディレクトリ構成より古い。

特に02は現行 `はじめてのギャルギルド` を優先する。

### 2. MarkdownとHTMLの同期を前提にしない

既存の `.html` は、現行 `.md` より古い場合がある。

本文監査では `.md` を正とし、HTMLの文字量や内容を見て「本文が薄い」と誤判定しない。逆に、`.md` が長くても文章品質が十分とはみなさない。

### 3. seedスクリプトは現行202604へ再実行しない

`tools/seed_202604_articles.mjs` は初期雛形生成用で、古い記事一覧と汎用定型文を含む。

Astraの編集開始時にこれを実行して現行202604を再シードしない。

---

## Astraがこの表を更新するとき

作業中に状態を更新する場合は、曖昧な `DONE` を避け、最低限次のように分ける。

- `AUDITED`：監査済み、本文未改稿
- `REWRITTEN_PENDING_USER`：改稿済み、ユーザー未確認
- `USER_REVIEWED`：ユーザーが内容確認済み
- `LAYOUT_SYNCED`：派生HTML/固定レイアウト同期済み
- `BUILD_VERIFIED`：号全体ビルド・検査済み

**本文を書いただけで `USER_REVIEWED` や `BUILD_VERIFIED` にしない。**
