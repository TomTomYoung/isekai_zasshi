...
状態：既存実装、利用者から不調報告、原因未特定。

リポジトリ直下の `index.html` と `codeswing.json` が入口です。記事を相対fetchで読み、1456×2056のsrcdoc iframeに表示します。保存後の再読込を意図しているため、成功すればレイアウト確認ごとのcommit/pushやPNG生成は不要です。本文HTMLを複製ファイルへ保存しません。

難点は、workspace.fsによるテキスト読取と、Webviewの画像/CSS/JS資産読取が別経路であることです。CodeSwingの代理URI、CSP、外部CDN、相対fetch、キャッシュを正しく扱う必要があります。現在の入口は任意のWebアプリを再現する汎用サーバーではなく、独自base・一部のRequest/URL形式・外部fetch等に制限があります。
...
通常はcommit/pushと公開反映を待つ方式です。devの未commit編集を自動で参照する機能ではありません。公開後も、現実装ではiframeの高さが文書全体へ変わる点や青枠の二重制御を修正・検証する必要があります。
...
状態：PNG用のNodeスクリプトには一時HTTPサーバーがある。ただし開発者が継続閲覧するサーバーとしては別途起動が必要。この監査では未実行。
...
現在の優先はAの実機切り分けです。HTTP基準試験Cを併用し、記事自体の破綻とCodeSwingの資産読取問題を分けます。Aが成立しない場合はE、または実行環境を許容するF/Gを検討します。Bは公開後確認、Hは最終PNGの手動アップロード削減に向きます。
